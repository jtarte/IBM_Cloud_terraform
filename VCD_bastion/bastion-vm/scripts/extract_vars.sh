...
echo "    \"lb_ip\":" $(echo $(cat $1 | grep internal_lb_ip| awk 'BEGIN {getline my_line; gsub(" ", "", my_line); print my_line}'| awk 'BEGIN {FS = "="} ;{ print $2}')) "," >> ./bastion-vm/ansible/ansible_vars.json
echo "    \"pull_secret\":" $(echo $(cat $1 | grep pull_secret_file| awk 'BEGIN {getline my_line; gsub(" ", "", my_line); print my_line}'| awk 'BEGIN {FS = "="} ;{ print $2}')) "," >> ./bastion-vm/ansible/ansible_vars.json
